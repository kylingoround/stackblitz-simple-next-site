...
//   ),
//   li: (props: any) => (
//     <li className="text-[16px] text[#2A2A2A] font-light text-2xl">{props.children}</li>
//   ),
...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    h1: ({ children }) => (
      <h1 className="text-[36px]">{children}</h1>
    ),
    ul: ({children}) => (<ul className='list-disc'>{children}</ul>),
    li: (props: any) => (
      <li className="leading-[1rem] mt-4 mb-4">{props.children}</li>
    ),
    strong: (props: any) => (<strong className='font-bold'>{props.children}</strong>),
    img: (props) => (
      <Image
